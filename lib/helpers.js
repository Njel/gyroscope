...
calcHours = function(start, end, allDay, supHH, supMM) {
  ...
  // return [{code: 'hrs', value: T},
  //         {code: 'N', value: N},
  //         {code: 'X100', value: X100},
  //         {code: 'X125', value: X125},
  //         {code: 'X150', value: X150},
  //         {code: 'X200', value: X200}];

  return results;
}

calcExtraHours = function(ev) {
  var post = Posts.findOne(ev.postId);
  var d0 = moment(ev.start);
  ...
  if (s) {
    ...
    if (p) {
      var m = moment(new Date(D + 'T' + p.end + ':00.000Z'));
      // console.log(m.toISOString());
      var supHH = m.get('hour');
      var supMM = m.get('minute');
    } else {
      var supHH = d0.get('hour');
      var supMM = d0.get('minute');
    }
    // console.log('supHH=' + supHH);
    // console.log('supMM=' + supMM);
  }
